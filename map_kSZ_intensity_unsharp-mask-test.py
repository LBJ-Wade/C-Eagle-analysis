...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins = 100, rfov = 2, output = 'show', title = True, plot_groups = 'FoF'):
    ...
    for i in [0, 1, 2]:
        ...
        smooth_factor = 0.1
        mask_kern = Gaussian2DKernel(x_stddev = smooth_factor*r200, y_stddev = smooth_factor*r200)
        aux_map = convolve(kSZmap, mask_kern)

        result_map = np.subtract(kSZmap, aux_map)
