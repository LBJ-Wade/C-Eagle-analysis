...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins = 100, rfov = 2, output = 'show', title = True, plot_groups = 'FoF'):
    ...
    # Load subhalo catalogue and obtain markers
    sg_catalougue = np.load('subfind_catalog_FullData.npy')
    print(sg_catalougue)
    sg_xyz = sgmark.subhalo_marks_filtered(fsgtab[0], fsgtab[1], sg_catalougue['index_catalog'])
