...
x = 2**np.abs(np.random.randn(10**5))
y = np.random.randn(10**5) + 2
...
gs = gridspec.GridSpec(ncols=5, nrows=7, 
						height_ratios=	[0.05,	1,		0.5, 	0.7, 	0.05,	1,	0.5], 
						width_ratios=	[1, 	0.5,	0.4, 	1, 		0.5])
...
data_dict['Vr'], _, _ = data_dict['Vr']
data_dict['MV'], _, _ = data_dict['MV']



# SELECTION
selec_dict = {'SELECT_R_MIN': 1, 
			  'SELECT_R_MAX': 2, 
...
latex_catalog = 1
if latex_catalog:
	index_select = np.load('subhalo_catalog.npy')
	catalog_dict = {'index_catalog' : data_dict['index'][index_select],
					'R_catalog' : data_dict['R'][index_select],
					'M_catalog' : data_dict['M'][index_select],
					'Fg_catalog' : data_dict['Fg'][index_select],
					'Vr_catalog' : data_dict['Vr'][index_select],
					'MV_catalog' : data_dict['MV'][index_select]}
	import pandas as pd
	Panda_catalog = pd.DataFrame.from_dict(catalog_dict, orient='index')
	LaTeX_out = Panda_catalog.to_latex(index=True)
...
for j in [0,4]:
	for i in [0,3]:
		...
		ax1v.hist(y,bins=bins_LIN, orientation='horizontal', color='red', histtype='step', cumulative= True)
		...
		if i == 0: ax1h.set_xlabel(label_R)
		elif i == 3: ax1h.set_xlabel(label_M)
