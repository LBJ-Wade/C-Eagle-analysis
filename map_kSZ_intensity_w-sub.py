...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins, rfov):
    ...
    sgx = subgroup_cop[:,0] - group_CoP[0]
    sgy = subgroup_cop[:,1] - group_CoP[1]
    sgz = subgroup_cop[:,2] - group_CoP[2]
    ...
    sgx = profile.comoving_length(sgx, h, redshift)
    sgy = profile.comoving_length(sgy, h, redshift)
    sgz = profile.comoving_length(sgz, h, redshift)

    sgm = profile.comoving_mass(subgroup_mass, h, redshift)
    sgm = profile.mass_units(sgm, unit_system = 'astro')
    #print('conversion to comoving')
    sgmom = profile.comoving_momentum(subgroup_momentum, h, redshift)
    sgmom = profile.momentum_units(sgmom, unit_system = 'astro')
    print('sgmom', np.shape(sgmom))
    ...
    # Subgroup selection
    sgindex = np.where((sgr < 5*r200) & (sgm > 0.08*sgm[1]))[0]
    ...
    for i in [0, 1, 2]:
        ...
        # Add subhalo markers
        axes[i].scatter(sgx_Data, sgy_Data, s = 14, marker = 'o', edgecolors = 'black', facecolors = 'None')
