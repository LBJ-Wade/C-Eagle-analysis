...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins=100, rfov=2, output='show', title=True,
    ...
    for i in [0]:
        ...
        img_par_scale = 0.75*np.max(kSZarray) # + (int(round(mp_rank))-2)/10
        img_par_offset = np.percentile(kSZarray, 75)/img_par_scale # + (int(round(mp_rank))-2)/10
    ...
    for i in [1]:
        ...
        for k in range(len(x)):
            circ = plt.Circle(((x[k] - nbins / 2 + 1) / nbins * rfov * r200 * 2, (y[k] - nbins / 2 + 1) / nbins * rfov * r200 * 2), r[k]/ nbins * rfov * r200 * 2,
    ...
    if output == 'show':
        ...
        file_name = 'blob_catalog.npy'
        save_name = 'kSZ-bblog-calib' + '_halo' + str(num_halo) + '_imgp-' + str('%3.1f' % img_par_offset).replace(".","") + '-' + str('%3.1f' % img_par_scale).replace(".", "") + '_sig-' + str('%4.2f' % min_sigma).replace(".", "") + '-' + str('%4.2f' % max_sigma).replace(".", "") + '_Nsig-' + str('%d' % num_sigma) + '_th-' + str('%4.2f' % threshold).replace(".","")
        ...
        catalog_dict['index_catalog'] = np.arange(len(x))
        catalog_dict['X'] = x
        catalog_dict['Y'] = y
        catalog_dict['R'] = r

        print(catalog_dict)
        np.save('blob_catalog', catalog_dict)
