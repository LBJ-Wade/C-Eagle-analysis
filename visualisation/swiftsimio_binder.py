...
if __name__ == '__main__':
    ...
    x = np.asarray(coords[:,0]/np.max(coords[:,0]), dtype = float64)
    y = np.asarray(coords[:,1]/np.max(coords[:,1]), dtype = float64)
    z = np.asarray(coords[:,2]/np.max(coords[:,2]), dtype = float64)
    m = np.asarray(kSZ, dtype = float32)
    h = np.asarray(SPH_kernel, dtype = float32)
    res = np.asarray(200, dtype = int)
