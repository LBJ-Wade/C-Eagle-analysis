...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins = 100, rfov = 2, output = 'show', title = True, plot_groups = 'FoF'):
    ...
    if output == 'show': 
        ...
        if not exists(dir_name): 
            makedirs(dir_name)
        save_name = 'kSZmap_' + plot_groups + '_halo'+str(num_halo) + '_z' + str(redshift_short).replace(".", "") + '_rfov' + str(rfov) + '_nbins' + str(nbins)
...
def map_kSZ_ICM(num_halo, redshift, simulation_type, nbins = 100, rfov = 2, output = 'show', title = True):
    map_kSZ_intensity(num_halo, redshift, simulation_type, nbins = nbins, rfov = rfov, output = output, title = True, plot_groups = 'FoF')

def map_kSZ_subgroups(num_halo, redshift, simulation_type, nbins = 100, rfov = 2, output = 'show', title = True):
    map_kSZ_intensity(num_halo, redshift, simulation_type, nbins = nbins, rfov = rfov, output = output, title = True, plot_groups = 'subgroups')

def call_kSZ_map(num_halo):
    simulation_type = 'gas'
    redshift = 0.57
    map_kSZ_ICM(num_halo, redshift, simulation_type, nbins = 200, rfov = 5, output = 'save', title = True)
    map_kSZ_subgroups(num_halo, redshift, simulation_type, nbins = 200, rfov = 5, output = 'save', title = True)
