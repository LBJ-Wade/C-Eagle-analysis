...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins=100, rfov=2, output='show', title=True,
    ...
    for i in [0]:
        ...
        aux_map = kSZmap
        #s_aux_map = 'aux_map = 1-np.abs(np.log10(np.abs(kSZmap)))/8'
        #_locals = locals()
        #exec(s_aux_map, globals(), _locals)
        #mapmin = np.min(np.log10(np.abs(kSZmap)))
        #mapmax = np.max((np.log10(np.abs(kSZmap)) - mapmin))
        print(mp_rank)
        aux2 = np.log10(np.abs(kSZmap))
    ...
    for i in [1]:
        ...
        for blob in blobs_log:
            y, x, r = blob
            circ = plt.Circle(((x - nbins / 2) / nbins * rfov * r200 * 2, (y - nbins / 2) / nbins * rfov * r200 * 2), r/ nbins * rfov * r200 * 2,
