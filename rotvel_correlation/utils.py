...
def medians_2d(x: np.ndarray, y: np.ndarray, axscales: List[str] = None, binning_method: str = None) -> dict:
    ...
    x_bin_stats = x_binning(x) if axscales[0] is 'linear' else 10 ** x_binning(np.log10(x))
...
output_labels = [
    ...
    '$\Omega_b$',
    '$\Omega_\Lambda$',
    r'$\theta(\mathbf{a_I}, \mathbf{J})$',
    r'$\theta(\mathbf{a_I}, \mathbf{v}_p)$',
    r'$\theta(\mathbf{a_I}, \mathbf{\omega})$',
    '$\mathbf{J}$',
    '$\mathbf{\omega}$',
    '$M_\mathrm{aperture}$',
    r'$\theta(\mathbf{b_I}, \mathbf{J})$',
    r'$\theta(\mathbf{b_I}, \mathbf{v}_p)$',
    r'$\theta(\mathbf{b_I}, \mathbf{\omega})$',
    r'$\theta(\mathbf{c_I}, \mathbf{J})$',
    r'$\theta(\mathbf{c_I}, \mathbf{v}_p)$',
    r'$\theta(\mathbf{c_I}, \mathbf{\omega})$',
    'Centre of mass',
    'Centre of potential',
    'Circular velocity',
