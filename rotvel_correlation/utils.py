...
def output_labels_F(parTtype:str=None)->str:
	...
	output_labels = [
			f"""$N_\\mathrm{{particles}}$""",
			f"""$N_{{sub}}$""",
			f"""$\\Omega_0$""",
			f"""$\\Omega_b$""",
			f"""$\\Omega_\\Lambda$""",
			f"""$|\\mathbf{{a}}_\\mathcal{{I}} \\cdot \\mathbf{{J}}|""",
			f"""$|\\mathbf{{a}}_\\mathcal{{I}} \\cdot \\mathbf{{v}}_p|""",
			f"""$|\\mathbf{{a}}_\\mathcal{{I}} \\cdot \\mathbf{{\\omega}}|""",
			f"""$\\mathbf{{J}}$""",
			f"""$\\mathbf{{\\omega}}$""",
			f"""$M_\\mathrm{{aperture}}\\quad$[M$_\\odot$]""",
			f"""$\\theta(\\mathbf{{b}}_\\mathcal{{I}}, \\mathbf{{J}})\\quad$[degrees]""",
			f"""$\\theta(\\mathbf{{b}}_\\mathcal{{I}}, \\mathbf{{v}}_p)\\quad$[degrees]""",
			f"""$\\theta(\\mathbf{{b}}_\\mathcal{{I}}, \\mathbf{{\\omega}})\\quad$[degrees]""",
			f"""$\\theta(\\mathbf{{c}}_\\mathcal{{I}}, \\mathbf{{J}})\\quad$[degrees]""",
			f"""$\\theta(\\mathbf{{c}}_\\mathcal{{I}}, \\mathbf{{v}}_p)\\quad$[degrees]""",
			f"""$\\theta(\\mathbf{{c}}_\\mathcal{{I}}, \\mathbf{{\\omega}})\\quad$[degrees]""",
			f"""Centre of mass""",
			f"""Centre of potential""",
			f"""Circular velocity$\\quad$[km\\,s$^{{-1}}$]""",
			f"""Dynamical merging index""",
			f"""Eigenvalues""",
			f"""Eigenvectors""",
			f"""Elongation""",
			f"""h""",
			f"""$inertia_tensor$""",
			f"""$kinetic_energy$""",
			f"""$\\theta(\\mathbf{{J}}, \\mathbf{{\\omega}})\\quad$[degrees]""",
			f"""$M_{{200}}\\quad$[M$_\\odot$]""",
			f"""$M_{{2500}}\\quad$[M$_\\odot$]""",
			f"""$M_{{500}}\\quad$[M$_\\odot$]""",
			f"""$M_{{FoF}}\\quad$[M$_\\odot$]""",
			f"""$R_{{200}}\\quad$[Mpc]""",
			f"""$R_{{2500}}\\quad$[Mpc]""",
			f"""$R_{{500}}\\quad$[Mpc]""",
			f"""$R_\\mathrm{{aperture}}\\quad$[Mpc]""",
			f"""Redshift""",
			f"""$J/M$""",
			f"""Sphericity""",
			f"""Spin parameter""",
			f"""$M_{{sub}}/M$""",
			f"""$M_{{sub}}\\quad$[M$_\\odot$]""",
			f"""Thermal energy""",
			f"""Thermodynamic merging index""",
			f"""Triaxiality""",
			f"""$\\theta(\\mathbf{{v}}_p, \\mathbf{{J}})\\quad$[degrees]""",
			f"""$\\theta(\\mathbf{{v}}_p, \\mathbf{{\\omega}})\\quad$[degrees]""",
			f"""$\\mathbf{{v}}_{{p}}\\quad$[km\\,s$^{{-1}}$]"""
	]
