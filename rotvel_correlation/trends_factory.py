...
for entry_index, data_entry in enumerate(data_entries):
    ...
    plt.savefig(os.path.join(pathSave, "scatterplot__{}__{}__aperture{}.png".format(data_entry['x'], data_entry['y'], aperture_id)))
    ##################################################################################################
    # kde PLOTS #
    ##################################################################################################
    fig_kde = fig
    ...
    for ax_idx, axes in enumerate(ax_kde):
        if ax_idx == 0:
            ...
            axes.scatter(x, y, s=3, c=simstats_palette[ax_idx-1], alpha=0.2)
            axes.text(0.95, 0.95, f"\\textsc{{{attrs[ax_idx-1]['Simulation']}}}", transform=axes.transAxes, **axisinfo_kwargs)
    plt.savefig(os.path.join(pathSave, "kdeplot__{}__{}__aperture{}.png".format(data_entry['x'], data_entry['y'], aperture_id)))