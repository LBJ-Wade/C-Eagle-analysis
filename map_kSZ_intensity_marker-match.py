...
def map_kSZ_intensity(num_halo, redshift, simulation_type, nbins = 100, rfov = 5, output = 'show', title = True, plot_groups = 'FoF'):
    # Import data
    ...
    for i in [0, 1, 2]:
        ...
        blob_catalogue = substructure_identification(np.asarray(kSZmap), rfov, r200)
        ...
        axes[i].scatter(sg_xyz[ijk[i][0]], sg_xyz[ijk[i][1]], s = 20, marker='x', edgecolors='black', facecolors='black')
        ...
        for k in range(len(blx)):
            circ = plt.Circle((blx[k], bly[k]), blr[k], color='green', linewidth=1, fill=False)
    ...
    if output == 'show':
        ...
        dir_name = 'kSZ maps marker-match'
        save_name = 'kSZmap_' + plot_groups + '_mark-match_halo' + str(num_halo) + '_z' + str(redshift_short).replace(".","") + '_rfov' + str(rfov) + '_nbins' + str(nbins)
...
# $$$ CMD: >> mpiexec -n 10 python map_kSZ_intensity_marker-match.py
